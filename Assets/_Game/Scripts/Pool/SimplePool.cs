...
public static class SimplePool
{
    class Pool
    {
        ...
        bool m_collect;
        //list object in pool
        Queue<PoolMember> m_inactive;
        //collect obj active ingame
        HashSet<PoolMember> m_active;
        // The prefab that we are pooling
        PoolMember m_prefab;

        public bool IsCollect { get => m_collect; }
        public HashSet<PoolMember> Active => m_active;
        public int Count => m_inactive.Count + m_active.Count;
        public Transform Root => m_sRoot;

        // Constructor
        public Pool(PoolMember prefab, int initialQty, Transform parent, bool collect)
        {
            m_inactive = new Queue<PoolMember>(initialQty);
            m_sRoot = parent;
            this.m_prefab = prefab;
            m_collect = collect;
            if (m_collect) m_active = new HashSet<PoolMember>();
        }

        // Spawn an object from our pool with position and rotation
        public PoolMember Spawn(Vector3 pos, Quaternion rot)
        {
            PoolMember obj = Spawn();
        ...
        public PoolMember Spawn()
        {
            ...
            {
                obj = (PoolMember)GameObject.Instantiate(m_prefab, m_sRoot);
            }
            else
            {
        ...
        public void Release()
        {
            ...
            {
                PoolMember go = m_inactive.Dequeue();
        ...
        public void Collect()
        {
            ...
            HashSet<PoolMember> units = new HashSet<PoolMember>(m_active);
    ...
    static Dictionary<PoolType, PoolMember> poolTypes = new Dictionary<PoolType, PoolMember>();
    ...
    static public void Preload(PoolMember prefab, int qty = 1, Transform parent = null, bool collect = false)
    {
    ...
    // init pool
    static void InitPool(PoolMember prefab = null, int qty = DEFAULT_POOL_SIZE, Transform parent = null, bool collect = false)
    {
    ...
    static private bool IsHasPool(PoolMember obj)
    {
        return poolInstance.ContainsKey(obj.poolType);
    }
    static private Pool GetPool(PoolMember obj)
    {
        return poolInstance[obj.poolType];
    }
    public static PoolMember GetPrefabByType(PoolType poolType)
    {
        if (!poolTypes.ContainsKey(poolType) || poolTypes[poolType] == null)
        {
            PoolMember[] resources = Resources.LoadAll<PoolMember>("Pool");
    ...
    public static HashSet<PoolMember> GetAllUnitIsActive(PoolMember obj)
    {
        return IsHasPool(obj) ? GetPool(obj).Active : new HashSet<PoolMember>();
    }
    public static HashSet<PoolMember> GetAllUnitIsActive(PoolType poolType)
    {
    ...
    #region Spawn
    // Spawn Unit to use
    static public T Spawn<T>(PoolType poolType, Vector3 pos, Quaternion rot) where T : PoolMember
    {
        return Spawn(GetPrefabByType(poolType), pos, rot) as T;
    }
    static public T Spawn<T>(PoolType poolType) where T : PoolMember
    {
        return Spawn<T>(GetPrefabByType(poolType));
    }
    static public T Spawn<T>(PoolMember obj, Vector3 pos, Quaternion rot) where T : PoolMember
    {
        return Spawn(obj, pos, rot) as T;
    }
    static public T Spawn<T>(PoolMember obj) where T : PoolMember
    {
        return Spawn(obj) as T;
    }

    // spawn PoolMember with transform parent
    static public T Spawn<T>(PoolMember obj, Transform parent) where T : PoolMember
    {
        return Spawn<T>(obj, obj.tf.localPosition, obj.tf.localRotation, parent);
    }

    static public T Spawn<T>(PoolMember obj, Vector3 localPoint, Quaternion localRot, Transform parent) where T : PoolMember
    {
        T unit = Spawn<T>(obj);
    ...
    static public T Spawn<T> (PoolType poolType, Vector3 localPoint, Quaternion localRot, Transform parent) where T : PoolMember
    {
        return Spawn<T>(GetPrefabByType(poolType), localPoint, localRot, parent);
    }
     static public T Spawn<T> (PoolType poolType, Transform parent) where T : PoolMember
    {
        return Spawn<T>(GetPrefabByType(poolType), parent);
    }

    static public PoolMember Spawn(PoolMember obj, Vector3 pos, Quaternion rot)
    {
        if (!IsHasPool(obj))
        {
    ...
    static public PoolMember Spawn(PoolMember obj)
    {
        if (!IsHasPool(obj))
        {
            Transform newRoot = new GameObject(obj.name).transform;
    ...
    #region Despawn
    //take PoolMember to pool
    static public void Despawn(PoolMember obj)
    {
    ...
    #region Release
    //destroy pool
    static public void Release(PoolMember obj)
    {
        if (IsHasPool(obj))
        {
            GetPool(obj).Release();
    ...
    static public void Collect(PoolMember obj)
    {
